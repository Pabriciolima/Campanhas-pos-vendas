...
/* Relatório visual de pagamentos por filial — Produtivos + Pix | 2026.09.25 */
const ROOT="relatorioPagamentosFilial",STYLE=ROOT+"Style";
...
function label(c){const[y,m]=c.split("-").map(Number);return new Intl.DateTimeFormat("pt-BR",{month:"short",year:"2-digit"}).format(new Date(y,m-1,1)).replace(".","")}
function addCss(){if($("#"+STYLE))return;const e=document.createElement("style");e.id=STYLE;e.textContent=`
.${ROOT}{margin-top:18px} .${ROOT} .rp-head{display:flex;justify-content:space-between;align-items:flex-end;gap:16px;flex-wrap:wrap}
.${ROOT} h2{margin:2px 0 4px} .${ROOT} p{margin:0;color:var(--muted,#718390);font-size:12px}
.${ROOT} .rp-filter{display:grid;gap:5px;min-width:240px} .${ROOT} .rp-filter span{font-size:10px;font-weight:900;text-transform:uppercase}
.${ROOT} select{min-height:40px;padding:0 12px;border:1px solid var(--line,#dce6ec);border-radius:10px;background:var(--surface,#fff);color:inherit}
.${ROOT} .rp-kpis{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px;margin:18px 0}
.${ROOT} .rp-kpi{padding:14px;border:1px solid var(--line,#dce6ec);border-radius:14px;background:rgba(127,127,127,.04)}
.${ROOT} .rp-kpi span{display:block;font-size:10px;font-weight:800;text-transform:uppercase;color:var(--muted,#718390)} .${ROOT} .rp-kpi strong{display:block;margin-top:6px;font-size:19px}
.${ROOT} .rp-chart{padding:16px 12px 6px;border:1px solid var(--line,#dce6ec);border-radius:16px;overflow-x:auto}
.${ROOT} svg{display:block;width:100%;min-width:620px;height:260px}.rp-grid{stroke:currentColor;opacity:.1}.rp-line{fill:none;stroke:#12a985;stroke-width:3}.rp-area{fill:#12a985;opacity:.08}.rp-dot{fill:#12a985;stroke:var(--surface,#fff);stroke-width:3}.rp-lab{fill:currentColor;font-size:10px;opacity:.72}.rp-val{fill:currentColor;font-size:10px;font-weight:800}
